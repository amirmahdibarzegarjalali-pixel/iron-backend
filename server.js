...
function render(){
  ...
  app.innerHTML='<div style="max-width:900px;margin:0 auto;">'+
    ...
    '<div class="panel"><h3>🎫 تیکت‌های پشتیبانی</h3>'+
      '<select id="ticketFilterStatus" onchange="renderTicketsList()" style="width:auto;display:inline-block;"><option value="">همه‌ی وضعیت‌ها</option><option value="open">باز</option><option value="answered">پاسخ داده‌شده</option><option value="closed">بسته</option></select>'+
      '<div id="ticketsList" style="margin-top:10px;">در حال بارگذاری...</div>'+
    ...
    '</div>'+
  '</div>';
  loadStats(); loadUsers(); loadCodes(); loadPlans(); loadDiscounts(); loadPayments(); loadAiSettings(); loadCommission(); loadAllCoaches(); loadCoachReports(); loadCoachPlansEditor(); loadCoachPrograms(); loadTickets(); loadCoachWallets();
}
...
function renderCoachWalletsTable(){
  ...
  "</div>";
  html+="<table><tr><th>مربی</th><th>موجودی</th><th>درآمد کل</th><th>کمیسیون پرداختی</th><th>تعداد تراکنش</th><th></th></tr>";
  list.forEach(function(w){
    html+="<tr><td>"+esc(w.coachName)+"</td>"+
      ...
      "<td>"+w.txCount+"</td>"+
      "<td><button class='small' onclick=openCoachWallet('"+w.coachId+"')>مشاهده</button></td></tr>";
  });
  html+="</table>";
...
function renderCoachWalletDetail(){
  ...
  let html="<div style='border:1.5px solid rgba(216,174,82,.3);border-radius:10px;padding:14px;'>"+
    ...
    "</div>"+
    "<h3 style='font-size:13px;'>تاریخچه‌ی تراکنش‌ها</h3>"+
    "<table><tr><th>تاریخ</th><th>نوع</th><th>مبلغ</th><th>توضیح</th></tr>";
  const txs=(w.transactions||[]).slice().reverse();
  const typeLabel={earning:"درآمد",commission:"کمیسیون",withdrawal:"برداشت",adjustment_credit:"تعدیل (+)",adjustment_debit:"تعدیل (-)"};
  txs.forEach(function(t){
    const pos=t.type==="earning"||t.type==="adjustment_credit";
    html+="<tr><td style='font-size:10px;'>"+new Date(t.date).toLocaleDateString("fa-IR")+"</td>"+
      ...
      "<td style='font-size:10px;'>"+esc(t.desc||"")+(t.type==="withdrawal"?(t.paid?" <span class='badge on'>پرداخت‌شده</span>":" <button class='small' onclick=markWalletTxPaid('"+c.id+"','"+t.id+"')>ثبت پرداخت</button>"):"")+"</td></tr>";
  });
  html+="</table>"+
    ...
    "<button onclick=submitWalletAdjust('"+c.id+"')>ثبت تعدیل</button>"+
  "</div>";
...
async function markWalletTxPaid(coachId,txId){
  try{
    await api("/api/admin/coach-wallets/"+coachId+"/mark-paid",{method:"POST",body:JSON.stringify({txId})});
    openCoachWallet(coachId);
  }catch(e){alert(e.message);}
}
async function loadTickets(){
  try{
    window._allTickets=await api("/api/admin/tickets");
    renderTicketsList();
  }catch(e){document.getElementById("ticketsList").innerHTML="<div style='color:#FF5252;'>"+esc(e.message)+"</div>";}
}
function renderTicketsList(){
  const filt=document.getElementById("ticketFilterStatus").value;
  let list=window._allTickets||[];
  if(filt) list=list.filter(function(t){return t.status===filt;});
  ...
  const statusMeta={open:["badge warn","باز"],answered:["badge on","پاسخ داده‌شده"],closed:["badge off","بسته"]};
  let html="<table><tr><th>موضوع</th><th>کاربر</th><th>دسته</th><th>وضعیت</th><th></th></tr>";
  list.forEach(function(t){
    const sm=statusMeta[t.status]||["badge","-"];
    html+="<tr><td>"+esc(t.subject)+"</td><td>"+esc(t.userName)+"</td>"+
      "<td style='font-size:10px;'>"+esc(t.category)+"</td>"+
      "<td><span class='"+sm[0]+"'>"+sm[1]+"</span></td>"+
      "<td><button class='small' onclick=openTicket('"+t.id+"')>باز کردن</button></td></tr>";
  });
  html+="</table>";
...
function renderTicketDetail(){
  ...
  let html="<div style='border:1.5px solid rgba(216,174,82,.3);border-radius:10px;padding:14px;'>"+
    "<h3>"+esc(t.subject)+"</h3>"+
    "<div style='font-size:11px;color:#9A9484;margin-bottom:8px;'>کاربر: "+esc(t.userName)+" ("+esc(t.userEmail||"-")+") · تلفن: "+esc(t.userPhone||"-")+" · دسته: "+esc(t.category)+"</div>"+
  ...
  msgs.forEach(function(m){
    ...
    html+="<div style='align-self:"+(mine?"flex-end":"flex-start")+";max-width:78%;background:"+(mine?"rgba(216,174,82,.15)":"rgba(255,255,255,.04)")+";border-radius:10px;padding:8px 10px;'>"+
      "<div style='font-size:9px;color:#9A9484;margin-bottom:3px;'>"+(mine?"👨‍💼 پشتیبانی":"👤 کاربر")+"</div>"+
      (m.attachmentUrl?"<a href='"+m.attachmentUrl+"' target='_blank'><img class='thumb' src='"+m.attachmentUrl+"' style='margin-bottom:4px;display:block;'></a>":"")+
      (m.text?"<div style='font-size:12px;white-space:pre-wrap;'>"+esc(m.text)+"</div>":"")+
    "</div>";
  });
  html+="</div>"+
    "<textarea id='ticketReplyText' placeholder='پاسخت رو بنویس...' style='min-height:60px;'></textarea>"+
    "<input type='file' id='ticketReplyImage' accept='image/png,image/jpeg,image/webp'>"+
    ...
    "<select id='ticketReplyStatus' style='width:auto;'>"+
      ...
      "<option value='open'"+(t.status==="open"?" selected":"")+">باز بمونه</option>"+
      "<option value='closed'"+(t.status==="closed"?" selected":"")+">بسته بشه</option>"+
    "</select>"+
    "<button onclick=sendTicketReply()>ارسال پاسخ</button>"+
    "</div>"+
  "</div>";
...
async function sendTicketReply(){
  ...
  const fileInput=document.getElementById("ticketReplyImage");
  const status=document.getElementById("ticketReplyStatus").value;
  const hasFile=fileInput.files&&fileInput.files[0];
  if(!text&&!hasFile){alert("پیام یا عکس رو وارد کن");return;}
  const fd=new FormData();
  fd.append("message",text); fd.append("status",status);
  if(hasFile) fd.append("image0",fileInput.files[0]);
  try{
    await apiUpload("/api/admin/tickets/"+window._currentTicket.id+"/reply",fd);
    await openTicket(window._currentTicket.id);
    loadTickets();
  }catch(e){alert(e.message||"خطا در ارسال");}
}
...
async function loadAllCoaches(){
  try{
    ...
    list.forEach(function(c){
      ...
      html+='<tr><td>'+esc(c.name||"-")+'</td><td>'+statusBadge+'</td>'+
...
adminRoutes.get("/coach-settings", (req, res, next) => {
  try { requireAdminUser(req.userId); res.json({ commissionRate: getCoachCommissionRate() }); } catch (e) { next(e); }
});
adminRoutes.put("/coach-settings", (req, res, next) => {
  try { requireAdminUser(req.userId); res.json({ commissionRate: setCoachCommissionRate(req.body.commissionRate) }); } catch (e) { next(e); }
});
...
adminRoutes.get("/coach-plans", (req, res, next) => { try { requireAdminUser(req.userId); res.json(getCoachPlans()); } catch (e) { next(e); } });
adminRoutes.put("/coach-plans", (req, res, next) => { try { requireAdminUser(req.userId); res.json(setCoachPlans(req.body.plans)); } catch (e) { next(e); } });
...
adminRoutes.get("/coach-wallets/:coachId", (req, res, next) => {
  try {
    ...
    res.json({ coach: { id: coach.id, name: coach.name, status: coach.status }, wallet: w });
  } catch (e) { next(e); }
});
...
adminRoutes.post("/coach-wallets/:coachId/mark-paid", (req, res, next) => {
  try {
    requireAdminUser(req.userId);
    const coach = CoachRepo.findById(req.params.coachId);
    if (!coach) throw httpError(404, "مربی پیدا نشد");
    ...
    if (!tx) throw httpError(404, "تراکنش پیدا نشد");
    tx.paid = true;
    _coachWallet.update(w.id, { transactions: w.transactions });
    res.json(w);
  } catch (e) { next(e); }
});
...
adminRoutes.get("/coach-programs", (req, res, next) => {
  try {
    ...
    const list = CoachProgramRepo.all().map((p) => {
      ...
      return Object.assign({}, p, { coachName: c ? c.name : "-" });
    });
    res.json(list.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
  } catch (e) { next(e); }
});
...
function publicStudentRow(row) {
  ...
  return Object.assign({}, row, { userName: u ? u.name : "کاربر", userAvatar: u ? u.avatarUrl : null });
}
...
AdminController.approvePayment = function(adminId, paymentId) {
  ...
  if (p.meta && p.meta.type === "coach_booking" && p.meta.coachId) {
    const commission = Math.round((p.amountToman || 0) * getCoachCommissionRate());
    const earning = (p.amountToman || 0) - commission;
...
const TICKET_CATEGORIES = ["فنی", "مالی و پرداخت", "گزارش مربی", "اشتراک VIP", "سایر"];
function saveTicketImage(file) {
  const ALLOWED = ["image/png", "image/jpeg", "image/webp"];
  if (!ALLOWED.includes(file.mimeType)) throw httpError(400, "فرمت عکس باید png، jpg یا webp باشد");
  if (file.size > 5 * 1024 * 1024) throw httpError(400, "حجم عکس نباید بیشتر از ۵ مگابایت باشد");
  const ext = file.mimeType.split("/")[1] || "jpg";
...
ticketRoutes.post("/", (req, res, next) => {
  try {
    const { subject, category, message, coachId } = req.body;
    ...
    const cat = TICKET_CATEGORIES.includes(category) ? category : "سایر";

    let imageUrl = null;
    const files = req.files || {};
    if (files.image0) imageUrl = saveTicketImage(files.image0);

    const ticket = TicketRepo.create({
      userId: req.userId, subject: String(subject).slice(0, 150), category: cat,
      status: "open", priority: "normal", coachId: coachId || null, lastMessageAt: new Date().toISOString(),
    });
    TicketMessageRepo.create({
      ticketId: ticket.id, senderRole: "user", text: String(message).slice(0, 3000), attachmentUrl: imageUrl,
    });
    SecurityLog.log({ userId: req.userId, type: "ticket_created", meta: { ticketId: ticket.id, category: cat } });
...
ticketRoutes.post("/:id/reply", (req, res, next) => {
  try {
    ...
    const files = req.files || {};
    if (files.image0) imageUrl = saveTicketImage(files.image0);
    if (!isNonEmptyString(message) && !imageUrl) throw httpError(400, "پیام خالیه");
    const msg = TicketMessageRepo.create({ ticketId: ticket.id, senderRole: "user", text: message ? String(message).slice(0, 3000) : "", attachmentUrl: imageUrl });
    TicketRepo.update(ticket.id, { status: "open", lastMessageAt: new Date().toISOString() });
    res.status(201).json(msg);
  } catch (e) { next(e); }
});
...
adminRoutes.get("/tickets", (req, res, next) => {
  try {
    requireAdminUser(req.userId);
    const list = TicketRepo.all().map((t) => {
      const u = UserRepo.findById(t.userId);
      return Object.assign({}, t, { userName: u ? u.name : "-", userEmail: u ? u.email : "-" });
    });
...
adminRoutes.post("/tickets/:id/reply", (req, res, next) => {
  try {
    ...
    if (files.image0) imageUrl = saveTicketImage(files.image0);
    if (!isNonEmptyString(message) && !imageUrl) throw httpError(400, "پیام خالیه");
    const msg = TicketMessageRepo.create({ ticketId: ticket.id, senderRole: "admin", text: message ? String(message).slice(0, 3000) : "", attachmentUrl: imageUrl });
    const patch = { lastMessageAt: new Date().toISOString() };
    if (status && ["open", "answered", "closed"].includes(status)) patch.status = status;
...
adminRoutes.put("/tickets/:id/status", (req, res, next) => {
  try {
    requireAdminUser(req.userId);
    const { status } = req.body;
    if (!["open", "answered", "closed"].includes(status)) throw httpError(400, "وضعیت نامعتبر است");
    const ticket = TicketRepo.update(req.params.id, { status });
    if (!ticket) throw httpError(404, "تیکت پیدا نشد");
    res.json(ticket);
  } catch (e) { next(e); }
});
