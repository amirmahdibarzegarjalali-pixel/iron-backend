...
function render(){
  ...
  app.innerHTML='<div style="max-width:900px;margin:0 auto;">'+
    ...
    '</div>'+
  '</div>';
  loadStats(); loadUsers(); loadCodes(); loadPlans(); loadDiscounts(); loadPayments(); loadAiSettings(); loadCommission(); loadAllCoaches(); loadCoachReports(); loadCoachPlansEditor();
}
...
async function loadAllCoaches(){
  try{
    ...
    let html='<table><tr><th>نام</th><th>وضعیت</th><th>شاگرد</th><th>فروش</th><th>درآمد کل</th><th>رتبه</th><th>عملیات</th><th>اعلان</th></tr>';
    list.forEach(function(c){
      const statusBadge=c.status==="approved"?'<span class="badge on">تاییدشده</span>':c.status==="banned"?'<span class="badge off">مسدود</span>':c.status==="pending"?'<span class="badge">در انتظار</span>':'<span class="badge off">'+esc(c.status)+'</span>';
      html+='<tr><td>'+esc(c.name||"-")+'</td><td>'+statusBadge+'</td>'+
        ...
        '<td style="font-size:10px;">'+esc((c.ranking&&c.ranking.rank)||"-")+'</td>'+
        '<td>'+(c.status==="banned"?
...
async function loadCoachReports(){
  try{
    ...
    list.forEach(function(r){
      html+="<div style='padding:10px;border:1px solid rgba(255,255,255,.08);border-radius:6px;margin-bottom:6px;"+(r.status==="resolved"?"opacity:.5;":"")+"'>"+
        "<div><b>"+esc(r.coachName)+"</b> — گزارش از "+esc(r.userName)+"</div>"+
        "<div style='font-size:11px;color:#9A9484;margin-top:4px;'>"+esc(r.reason)+"</div>"+
        (r.status==="open"?"<button style='margin-top:6px;' onclick=resolveCoachReport(\\""+r.id+"\\")>✅ بررسی شد</button>":"<span class='badge on'>بررسی‌شده</span>")+
      "</div>";
    });
...
async function resolveCoachReport(id){
  try{await api("/api/admin/coach-reports/"+id+"/resolve",{method:"PUT"});loadCoachReports();}catch(e){alert(e.message);}
}
...
coachRoutes.post("/report", (req, res, next) => {
  try {
    ...
    const row = CoachReportRepo.create({ coachId, userId: req.userId, reason: String(reason).slice(0, 1000), status: "open" });
