...
<style>
:root{--bg:#0A0B09;--panel:#131913;--border:rgba(76,122,82,.3);--brass:#D8AE52;--green2:#3DDC74;--ink2:#FF5252;--text:#F3EEDD;--muted:#9A9484;}
...
input,select{background:var(--bg);color:var(--text);border:1px solid var(--border);border-radius:8px;padding:8px 12px;font-family:inherit;font-size:13px;width:100%;margin-bottom:8px;}
input:focus{outline:none;border-color:var(--brass);}
.btn{padding:8px 16px;border-radius:8px;border:none;cursor:pointer;font-family:inherit;font-size:13px;font-weight:700;}
.btn-primary{background:var(--brass);color:#1A140B;}
.btn-success{background:var(--green);color:#fff;}
.btn-danger{background:var(--ink2);color:#fff;}
...
.badge.vip{background:rgba(216,174,82,.15);color:var(--brass);}
.err{color:var(--ink2);font-size:12px;margin-top:8px;}
.empty{text-align:center;color:var(--muted);padding:24px;}
.sidebar{position:fixed;top:0;right:0;width:240px;height:100vh;background:var(--panel);border-left:1px solid var(--border);overflow-y:auto;z-index:100;}
.sidebar-header{padding:20px 16px;border-bottom:1px solid var(--border);}
.sidebar-header h1{font-size:18px;color:var(--brass);}
.nav-item{display:block;padding:12px 16px;cursor:pointer;color:var(--muted);font-size:13px;text-decoration:none;border-right:3px solid transparent;}
.nav-item:hover{background:rgba(216,174,82,.06);color:var(--text);}
.nav-item.active{background:rgba(216,174,82,.1);color:var(--brass);border-right-color:var(--brass);}
.main{margin-right:240px;padding:16px;}
...
<div id="app"><div style="text-align:center;padding:40px;"><div class="spinner"></div></div></div>
<script>
var T={access:null};
async function api(path,opts){
  ...
  var res=await fetch(path,Object.assign({headers:h},opts));
...
var me=null,err="",section="dashboard";
var NAV=[
  ["dashboard","📊","داشبورد"],["users","👥","کاربران"],["payments","💰","پرداخت‌ها"],
  ["vip","🎟️","کدهای VIP"],["plans","📋","پلن‌ها"],["discounts","🏷️","تخفیف"],["ai","🤖","هوش مصنوعی"]
];
function render(){
  var app=document.getElementById("app");
  if(!me){
    app.innerHTML="<div class=card style='max-width:360px;margin:60px auto'>"+
      "<h2 style='text-align:center;margin-bottom:16px'>🔐 ورود مدیر</h2>"+
    ...
    document.getElementById("email").onkeydown=function(e){if(e.key==="Enter")document.getElementById("pw").focus();};
    return;
  }
  var navHtml="<div class=sidebar-header><h1>🏋️ آیرون</h1><small style='font-size:11px;color:var(--muted)'>پنل مدیریت</small></div>";
  NAV.forEach(function(n){
    navHtml+="<a class='nav-item "+(section===n[0]?"active":"")+"' data-sec='"+n[0]+"'>"+n[1]+" "+n[2]+"</a>";
  });
  navHtml+="<div style='padding:12px 16px;border-top:1px solid var(--border);margin-top:20px'>"+
    "<div style='font-size:11px;color:var(--muted);margin-bottom:4px'>"+esc(me.name||me.email||"")+"</div>"+
    "<a class='nav-item' data-sec='logout' style='color:var(--ink2)'>🚪 خروج</a></div>";
  var curTitle=NAV.find(function(n){return n[0]===section;});
  app.innerHTML="<div class=sidebar id=sidebar>"+navHtml+"</div>"+
    "<div class=main><div class=topbar><div style='font-size:16px;font-weight:700'>"+(curTitle?curTitle[2]:"")+
    "</div><div style='font-size:11px;color:var(--muted)'>"+new Date().toLocaleDateString("fa-IR")+"</div></div>"+
    "<div id=content><div style='text-align:center;padding:20px'><div class=spinner></div></div></div></div>";
  document.querySelectorAll("[data-sec]").forEach(function(el){
    el.onclick=function(){
      var s=el.getAttribute("data-sec");
      if(s==="logout"){me=null;T.access=null;render();return;}
      section=s;render();loadSection();
    };
  });
  loadSection();
}
async function doLogin(){
  ...
  if(!email||!pw){err="ایمیل و رمز رو وارد کن";render();return;}
  try{
    var res=await api("/api/auth/login",{method:"POST",body:JSON.stringify({email:email,password:pw})});
    if(res.user.role!=="admin"){err="دسترسی مدیریت نداری";render();return;}
    T.access=res.accessToken;me=res.user;render();
  }catch(e){err=e.message;render();}
}
function sc(n,l,c){return "<div class='stat-card "+c+"'><div class=stat-num>"+fa(n)+"</div><div class=stat-label>"+l+"</div></div>";}
function loadSection(){
  var el=document.getElementById("content");
  if(section==="dashboard") loadDash(el);
  else if(section==="users") loadU(el);
  else if(section==="payments") loadP(el);
  else if(section==="vip") loadV(el);
  else if(section==="plans") loadPl(el);
  else if(section==="discounts") loadD(el);
  else if(section==="ai") loadA(el);
}
async function loadDash(el){
  el.innerHTML="<div class=spinner></div>";
  try{
    var s=await api("/api/admin/stats");
    var a;try{a=await api("/api/admin/ai-stats");}catch(e){a={todayMessages:0,totalMessages:0,distinctUsers:0,totalTokens:0,settings:{}};}
    el.innerHTML="<div class=stat-grid>"+
      sc(s.totalUsers,"کل کاربران","")+sc(s.vipUsers,"VIP","")+sc(s.signupsToday,"امروز","")+
      "</div><div class=stat-grid>"+
      sc(a.todayMessages||0,"پیام AI امروز","")+sc(a.totalMessages||0,"کل AI","")+sc(a.distinctUsers||0,"کاربران AI","")+
      "</div><div class=card><b>🤖 وضعیت AI</b><br>مدل: <b>"+esc(a.settings.model||"-")+"</b> | "+
      "<span class='badge "+(a.settings.enabled?"on":"off")+"'>"+(a.settings.enabled?"فعال":"غیرفعال")+"</span> | "+
      "محدودیت رایگان: <b>"+fa(a.settings.freeDailyLimit||0)+"</b></div>";
  }catch(e){el.innerHTML="<div class=card err>"+esc(e.message)+"</div>";}
}
async function loadU(el){
  el.innerHTML="<div class=card><input id=usearch placeholder='🔍 جستجو...'></div><div id=utable><div class=spinner></div></div>";
  var st;
  document.getElementById("usearch").oninput=function(){clearTimeout(st);st=setTimeout(loadU2,300);};
  loadU2();
}
async function loadU2(){
  var el=document.getElementById("utable");if(!el)return;
  el.innerHTML="<div class=spinner></div>";
  try{
    ...
    var h="<div class=scroll-table><table><tr><th>نام</th><th>ایمیل</th><th>VIP</th><th>عملیات</th></tr>";
    users.forEach(function(u){
      var v=u.vip&&u.vip.active;
      h+="<tr><td>"+esc(u.name)+"</td><td style='font-size:11px'>"+esc(u.email||u.phone||"-")+"</td>"+
        "<td><span class='badge "+(v?"on":"off")+"'>"+(v?"فعال":"-")+"</span></td>"+
        "<td><button class='btn btn-sm "+(v?"btn-danger":"btn-success")+"' data-vip='"+u.id+"' data-act='"+(v?"revoke":"gift")+"'>"+
        (v?"لغو":"هدیه")+"</button></td></tr>";
    });
    el.innerHTML=h+"</table></div>";
    el.querySelectorAll("[data-vip]").forEach(function(b){
      b.onclick=async function(){
        var id=b.getAttribute("data-vip");
        if(b.getAttribute("data-act")==="gift"){
          var d=prompt("چند روز؟","30");if(!d)return;
          try{await api("/api/admin/users/"+id+"/vip",{method:"PUT",body:JSON.stringify({active:true,days:Number(d)})});loadU2();}catch(e){alert(e.message);}
        }else{
          if(!confirm("مطمئنی؟"))return;
          try{await api("/api/admin/users/"+id+"/vip",{method:"PUT",body:JSON.stringify({active:false})});loadU2();}catch(e){alert(e.message);}
        }
      };
    });
  }catch(e){el.innerHTML="<div class=err>"+esc(e.message)+"</div>";}
}
async function loadP(el){
  el.innerHTML="<div class=spinner></div>";
  try{
    var p=await api("/api/admin/payments/pending");
    if(!p.length){el.innerHTML="<div class=card><div class=empty>پرداخت در انتظاری نیست ✅</div></div>";return;}
    var h="<div class=card><b>💰 در انتظار تایید</b><div class=scroll-table><table><tr><th>کاربر</th><th>روش</th><th>مبلغ</th><th>عملیات</th></tr>";
    p.forEach(function(pp){
      h+="<tr><td style='font-size:10px'>"+(pp.userId||"").slice(0,8)+"</td><td>"+(pp.provider==="usdt_wallet"?"تتر":"درگاه")+"</td>"+
        "<td>"+(pp.amountUsdt?pp.amountUsdt+" USDT":(pp.amountToman?fa(pp.amountToman)+" ت":"-"))+"</td>"+
        "<td><button class='btn btn-sm btn-success' data-pay='"+pp.id+"' data-do='ok'>تایید</button> "+
        "<button class='btn btn-sm btn-danger' data-pay='"+pp.id+"' data-do='no'>رد</button></td></tr>";
    });
    el.innerHTML=h+"</table></div></div>";
    el.querySelectorAll("[data-pay]").forEach(function(b){
      b.onclick=async function(){
        var id=b.getAttribute("data-pay");
        if(b.getAttribute("data-do")==="ok"){
          if(!confirm("تایید؟"))return;
          try{await api("/api/admin/payments/"+id+"/approve",{method:"PUT"});loadP(el);}catch(e){alert(e.message);}
        }else{
          try{await api("/api/admin/payments/"+id+"/reject",{method:"PUT",body:JSON.stringify({reason:""})});loadP(el);}catch(e){alert(e.message);}
        }
      };
    });
  }catch(e){el.innerHTML="<div class=card err>"+esc(e.message)+"</div>";}
}
async function loadV(el){
  el.innerHTML="<div class=spinner></div>";
  try{
    var codes=await api("/api/admin/vip-codes");
    var h="<div class=card><b>🎟️ ساخت کد VIP</b>"+
      "<input id=vcode placeholder='کد (خالی=خودکار)'>"+
      "<input id=vdays type=number placeholder='مدت (روز) مثلا 30'>"+
      "<input id=vmax type=number placeholder='حداکثر استفاده (خالی=نامحدود)'>"+
      "<button class='btn btn-primary' id=vcreate>ساخت کد</button></div>";
    if(codes.length){
      h+="<div class=card><b>کدهای VIP ("+fa(codes.length)+")</b><div class=scroll-table><table><tr><th>کد</th><th>مدت</th><th>استفاده</th><th>وضعیت</th><th></th></tr>";
      codes.forEach(function(c){
        h+="<tr><td style='font-weight:700;color:var(--brass)'>"+esc(c.code)+"</td><td>"+fa(c.durationDays)+" روز</td>"+
          "<td>"+fa(c.usedCount)+" / "+(c.maxUses===null?"∞":fa(c.maxUses))+"</td>"+
          "<td><span class='badge "+(c.isActive?"on":"off")+"'>"+(c.isActive?"فعال":"غیر")+"</span></td>"+
          "<td><button class='btn btn-sm btn-outline' data-vt='"+c.id+"' data-vv='"+(!c.isActive)+"'>"+(c.isActive?"غیرفعال":"فعال")+"</button></td></tr>";
      });
      h+="</table></div></div>";
    }
    el.innerHTML=h;
    document.getElementById("vcreate").onclick=async function(){
      try{
        var r=await api("/api/admin/vip-codes",{method:"POST",body:JSON.stringify({code:document.getElementById("vcode").value,durationDays:Number(document.getElementById("vdays").value),maxUses:document.getElementById("vmax").value?Number(document.getElementById("vmax").value):null})});
        alert("کد ساخته شد: "+r.code);loadV(el);
      }catch(e){alert(e.message);}
    };
    el.querySelectorAll("[data-vt]").forEach(function(b){
      b.onclick=async function(){
        try{await api("/api/admin/vip-codes/"+b.getAttribute("data-vt")+"/toggle",{method:"PUT",body:JSON.stringify({isActive:b.getAttribute("data-vv")==="true"})});loadV(el);}catch(e){alert(e.message);}
      };
    });
  }catch(e){el.innerHTML="<div class=card err>"+esc(e.message)+"</div>";}
}
async function loadPl(el){
  el.innerHTML="<div class=spinner></div>";
  try{
    var plans=await api("/api/admin/plans");
    if(!plans.length){el.innerHTML="<div class=card><div class=empty>پلنی نیست</div></div>";return;}
    var h="<div class=card><b>📋 پلن‌ها</b><table><tr><th>پلن</th><th>تومان</th><th>تتر</th><th>روز</th><th></th></tr>";
    plans.forEach(function(p){
      h+="<tr><td>"+esc(p.title)+"</td><td><input style='width:80px' id=pt_"+p.id+" value="+p.priceToman+"></td>"+
        "<td><input style='width:60px' id=pu_"+p.id+" value="+p.priceUsdt+"></td><td>"+fa(p.durationDays)+"</td>"+
        "<td><button class='btn btn-sm btn-primary' data-plan='"+p.id+"'>ذخیره</button></td></tr>";
    });
    el.innerHTML=h+"</table></div>";
    el.querySelectorAll("[data-plan]").forEach(function(b){
      b.onclick=async function(){
        var id=b.getAttribute("data-plan");
        try{await api("/api/admin/plans/"+id,{method:"PUT",body:JSON.stringify({priceToman:Number(document.getElementById("pt_"+id).value),priceUsdt:Number(document.getElementById("pu_"+id).value)})});alert("ذخیره شد ✅");}catch(e){alert(e.message);}
      };
    });
  }catch(e){el.innerHTML="<div class=card err>"+esc(e.message)+"</div>";}
}
async function loadD(el){
  el.innerHTML="<div class=spinner></div>";
  try{
    var codes=await api("/api/admin/discount-codes");
    var h="<div class=card><b>🏷️ ساخت تخفیف</b>"+
      "<input id=dcode placeholder='کد (خالی=خودکار)'>"+
      "<input id=dpct type=number placeholder='درصد تخفیف'>"+
      "<input id=dmax type=number placeholder='حداکثر استفاده'>"+
      "<button class='btn btn-primary' id=dcreate>ساخت</button></div>";
    if(codes.length){
      h+="<div class=card><b>کدهای تخفیف</b><table><tr><th>کد</th><th>تخفیف</th><th>استفاده</th><th>وضعیت</th><th></th></tr>";
      codes.forEach(function(c){
        h+="<tr><td>"+esc(c.code)+"</td><td>"+(c.percentOff?fa(c.percentOff)+"%":(c.amountOff?fa(c.amountOff)+" ت":"-"))+"</td>"+
          "<td>"+fa(c.usedCount)+" / "+(c.maxUses===null?"∞":fa(c.maxUses))+"</td>"+
          "<td><span class='badge "+(c.isActive?"on":"off")+"'>"+(c.isActive?"فعال":"غیر")+"</span></td>"+
          "<td><button class='btn btn-sm btn-outline' data-dt='"+c.id+"' data-dv='"+(!c.isActive)+"'>تبدیل</button></td></tr>";
      });
      h+="</table></div>";
    }
    el.innerHTML=h;
    document.getElementById("dcreate").onclick=async function(){
      try{
        var r=await api("/api/admin/discount-codes",{method:"POST",body:JSON.stringify({code:document.getElementById("dcode").value,percentOff:Number(document.getElementById("dpct").value)||null,maxUses:document.getElementById("dmax").value?Number(document.getElementById("dmax").value):null})});
        alert("ساخته شد: "+r.code);loadD(el);
      }catch(e){alert(e.message);}
    };
    el.querySelectorAll("[data-dt]").forEach(function(b){
      b.onclick=async function(){
        try{await api("/api/admin/discount-codes/"+b.getAttribute("data-dt")+"/toggle",{method:"PUT",body:JSON.stringify({isActive:b.getAttribute("data-dv")==="true"})});loadD(el);}catch(e){alert(e.message);}
      };
    });
  }catch(e){el.innerHTML="<div class=card err>"+esc(e.message)+"</div>";}
}
async function loadA(el){
  ...
  try{
    ...
    el.innerHTML="<div class=card><b>🤖 تنظیمات AI</b>"+
      "<div style='padding:8px 0'>کلید API: <span class='badge "+(s.hasApiKey?"on":"off")+"'>"+(s.hasApiKey?"تنظیم شده":"تنظیم نشده")+"</span></div>"+
      "<label style='font-size:12px;color:var(--muted)'>مدل</label><input id=amodel value='"+esc(s.settings.model||"")+"'>"+
      "<label style='font-size:12px;color:var(--muted)'>محدودیت روزانه رایگان</label><input id=alimit type=number value="+(s.settings.freeDailyLimit||5)+">"+
      "<label style='font-size:12px;color:var(--muted)'>فعال‌سازی</label>"+
      "<div><label><input type=checkbox id=aenabled "+(s.settings.enabled?"checked":"")+"> هوش مصنوعی فعال باشه</label></div>"+
      "<button class='btn btn-primary' id=asave style='margin-top:8px'>💾 ذخیره</button></div>"+
      "<div class=stat-grid>"+sc(s.todayMessages||0,"پیام امروز","")+sc(s.totalMessages||0,"کل پیام","")+sc(s.distinctUsers||0,"کاربران AI","")+sc(s.totalTokens||0,"توکن","")+"</div>";
    document.getElementById("asave").onclick=async function(){
      try{
        await api("/api/admin/ai-settings",{method:"PUT",body:JSON.stringify({enabled:document.getElementById("aenabled").checked,model:document.getElementById("amodel").value,freeDailyLimit:Number(document.getElementById("alimit").value)})});
        alert("ذخیره شد ✅");loadA(el);
      }catch(e){alert(e.message);}
    };
  }catch(e){el.innerHTML="<div class=card err>"+esc(e.message)+"</div>";}
}
