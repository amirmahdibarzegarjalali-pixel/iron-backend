...
function render(){
  ...
  app.innerHTML='<div style="max-width:900px;margin:0 auto;">'+
    ...
    '</div>'+
  '</div>';
  loadStats(); loadUsers(); loadCodes(); loadPlans(); loadDiscounts(); loadPayments(); loadAiSettings(); loadCommission(); loadAllCoaches(); loadCoachReports();
}
...
async function sendCoachNotice(id){
  const el=document.getElementById("notice_"+id);
  const text=el.value.trim();
  if(!text)return;
  try{await api("/api/admin/coaches/"+id+"/notice",{method:"POST",body:JSON.stringify({text})});alert("اعلان ارسال شد ✅");el.value="";}catch(e){alert(e.message);}
}
