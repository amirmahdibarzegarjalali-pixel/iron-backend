...
function render(){
  ...
  app.innerHTML='<div style="max-width:960px;margin:0 auto;">'+
    ...
    '<div style="display:flex;gap:6px;overflow-x:auto;margin:10px 0 16px;padding-bottom:4px;">'+
      '<button class="tabbtn" data-tab-btn="dashboard">📊 داشبورد</button>'+
      '<button class="tabbtn" data-tab-btn="users">👤 کاربران</button>'+
      '<button class="tabbtn" data-tab-btn="coaches">🧑‍🏫 مربیان</button>'+
      '<button class="tabbtn" data-tab-btn="billing">💳 پرداخت و اشتراک</button>'+
      '<button class="tabbtn" data-tab-btn="tickets">🎫 تیکت‌ها</button>'+
      '<button class="tabbtn" data-tab-btn="ai">🧠 هوش مصنوعی</button>'+
    '</div>'+
    ...
    '</div>'+
  '</div>';
  showAdminTab(window._adminActiveTab||"dashboard");
  loadStats(); loadAuditLog(1); loadUsers(); loadCodes(); loadPlans(); loadDiscounts(); loadPayments(); loadAiSettings(); loadCommission(); loadAllCoaches(); loadCoachReports(); loadCoachPlansEditor(); loadCoachPrograms(); loadTickets(); loadQuickReplies(); loadCoachWallets();
}
...
async function loadUsers(){
  try{
    ...
    let html='<table><tr><th>نام</th><th>ایمیل/موبایل</th><th>نقش</th><th>وضعیت</th><th>عملیات</th></tr>';
...
function renderUserDetail(){
  ...
  let html="<div class='panel'>"+
    "<div style='display:flex;justify-content:space-between;align-items:center;'>"+
      "<h3 style='margin:0;'>👤 "+esc(u.name)+(u.blocked?" <span style='color:#FF5252;font-size:11px;'>(مسدود)</span>":"")+"</h3>"+
...
function renderTicketsList(){
  ...
  list.forEach(function(t){
    ...
    html+="<div class='report-card' data-tk-open='"+t.id+"' style='cursor:pointer;'>"+
      "<div style='display:flex;justify-content:space-between;align-items:center;gap:8px;'>"+
        "<b style='font-size:12.5px;'>#"+(t.ticketNumber||"-")+" — "+esc(t.subject)+"</b>"+
...
function renderTicketDetail(){
  ...
  let html="<div style='border:1.5px solid rgba(216,174,82,.3);border-radius:10px;padding:14px;'>"+
    ...
    "<div style='font-size:11px;color:#9A9484;margin-bottom:8px;'>کاربر: "+esc(t.userName||"-")+" ("+esc(t.userEmail||"-")+") · تلفن: "+esc(t.userPhone||"-")+" · دسته: "+esc(t.category||"-")+" · اولویت: "+(TICKET_PRIORITY_LABELS[t.priority]||t.priority||"-")+"</div>"+
