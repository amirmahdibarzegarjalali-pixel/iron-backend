...
<style>
:root{--bg:#0A0B09;--panel:#131913;--border:rgba(76,122,82,0.3);--brass:#D8AE52;--brass2:#F2CE7A;--green:#1F9D55;--green2:#3DDC74;--ink:#C23B3B;--ink2:#FF5252;--amber2:#F2CE7A;--text:#F3EEDD;--muted:#9A9484;}
*{box-sizing:border-box;}
body{font-family:Tahoma,'Vazirmatn',Arial,sans-serif;background:radial-gradient(circle at 50% -10%, rgba(61,220,116,0.08), transparent 55%), var(--bg);color:var(--text);margin:0;padding:0;min-height:100vh;}
.mono{font-family:'Courier New',monospace;}
button{cursor:pointer;background:none;border:none;color:inherit;font-family:inherit;}
input,select,textarea{background:transparent;color:var(--text);font-family:inherit;}
input:focus,button:focus-visible,select:focus{outline:2px solid var(--green2);outline-offset:2px;}
.panel{background:var(--panel);border:1px solid var(--border);border-radius:12px;}
.row{display:flex;align-items:center;gap:10px;}
.wrap{max-width:1180px;margin:0 auto;padding:0 18px;}
.topbar{position:sticky;top:0;z-index:30;background:rgba(10,11,9,.92);backdrop-filter:blur(10px);border-bottom:1px solid var(--border);}
.topbar-inner{display:flex;align-items:center;justify-content:space-between;padding:14px 18px;max-width:1180px;margin:0 auto;gap:12px;flex-wrap:wrap;}
.brand{display:flex;align-items:center;gap:10px;}
.brand .badge{width:38px;height:38px;border-radius:10px;border:2px solid var(--brass);display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 0 14px rgba(216,174,82,.35);}
.brand .t1{font-weight:900;font-size:15px;}
.brand .t2{font-size:10px;color:var(--muted);}
.role-pill{font-size:11px;padding:5px 10px;border-radius:20px;background:rgba(216,174,82,.12);color:var(--brass2);border:1px solid rgba(216,174,82,.35);}
.logout-btn{font-size:12px;color:var(--ink2);border:1px solid rgba(255,82,82,.35);padding:6px 12px;border-radius:8px;}
.navtabs{display:flex;gap:6px;overflow-x:auto;padding:10px 18px;max-width:1180px;margin:0 auto;scrollbar-width:none;}
.navtabs::-webkit-scrollbar{display:none;}
.navbtn{display:flex;align-items:center;gap:6px;padding:9px 14px;border-radius:10px;font-size:13px;font-weight:700;white-space:nowrap;color:var(--muted);border:1px solid transparent;}
.navbtn.active{background:rgba(216,174,82,.1);color:var(--brass2);border-color:rgba(216,174,82,.35);}
.content{max-width:1180px;margin:0 auto;padding:8px 18px 60px;}
.section-gap{margin-top:14px;}
.err-banner{background:rgba(255,82,82,.08);border:1px solid rgba(255,82,82,.3);color:var(--ink2);font-size:12px;padding:10px 14px;border-radius:10px;margin-top:10px;}
.kpi-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;margin-top:14px;}
.kpi-card{padding:14px;}
.kpi-card .lb{font-size:11px;color:var(--muted);margin-bottom:6px;}
.kpi-card .val{font-size:20px;font-weight:900;}
.table-wrap{overflow-x:auto;}
table.dtable{width:100%;border-collapse:collapse;font-size:12.5px;min-width:600px;}
table.dtable th{text-align:right;color:var(--muted);font-weight:700;padding:10px 12px;border-bottom:1px solid var(--border);white-space:nowrap;}
table.dtable td{padding:10px 12px;border-bottom:1px solid rgba(76,122,82,.15);white-space:nowrap;}
table.dtable tr:hover td{background:rgba(255,255,255,.02);}
.pill{font-size:10.5px;padding:3px 10px;border-radius:20px;font-weight:700;display:inline-block;}
.pill.ok{background:rgba(61,220,116,.12);color:var(--green2);}
.pill.warn{background:rgba(216,168,61,.15);color:var(--amber2);}
.pill.bad{background:rgba(255,82,82,.12);color:var(--ink2);}
.pill.muted{background:rgba(154,148,132,.12);color:var(--muted);}
.iconbtn2{padding:6px 10px;border-radius:8px;border:1px solid var(--border);font-size:11.5px;font-weight:700;}
.iconbtn2:hover{border-color:var(--brass);}
...
.btn{padding:10px 16px;border-radius:9px;font-size:13px;font-weight:700;background:var(--brass);color:#1A140B;}
.btn.ghost{background:transparent;border:1px solid var(--border);color:var(--text);}
.btn:disabled{opacity:.5;pointer-events:none;}
.search-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;}
.search-row input{flex:1;min-width:180px;padding:9px 12px;border:1px solid var(--border);border-radius:8px;font-size:13px;}
.empty-note{text-align:center;padding:24px 0;color:var(--muted);font-size:12.5px;}
.login-wrap{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;}
.login-card{width:100%;max-width:380px;padding:28px 24px;}
.plan-row{display:grid;grid-template-columns:1.2fr .8fr .6fr .5fr auto;gap:6px;align-items:center;font-size:12px;padding:8px 0;border-bottom:1px solid rgba(76,122,82,.15);}
.plan-row input{width:100%;padding:6px 8px;border:1px solid var(--border);border-radius:6px;font-size:12px;}
.roadmap-item{display:flex;gap:8px;font-size:12px;color:var(--muted);padding:5px 0;}
...
window.onerror=function(msg,src,line,col,err){
  ...
  if(box) box.innerHTML='<div style="padding:18px;color:#FF9E9E;background:#2a0e0e;font-family:monospace;white-space:pre-wrap;direction:ltr;text-align:left;font-size:12px;line-height:1.6;">JS ERROR CAUGHT:\n'+String(msg)+'\nfile: '+String(src)+'\nline: '+line+' col: '+col+'\n'+(err&&err.stack?String(err.stack):'(no stack)')+'</div>';
};
setTimeout(function(){
  var box=document.getElementById("app");
  if(box && box.innerHTML.indexOf("در حال بارگذاری")>=0){
    box.innerHTML='<div style="padding:18px;color:#FFD98E;background:#2a220e;font-family:monospace;white-space:pre-wrap;direction:ltr;text-align:left;font-size:11px;line-height:1.6;">اسکریپت اصلی اجرا نشد (سه ثانیه گذشت).\nuserAgent: '+navigator.userAgent+'</div>';
  }
},3000);
var T={access:null};
function esc(s){ return String(s==null?"":s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"); }
function toFa(n){ var map={"0":"۰","1":"۱","2":"۲","3":"۳","4":"۴","5":"۵","6":"۶","7":"۷","8":"۸","9":"۹"}; return String(n==null?0:n).replace(/[0-9]/g,function(d){return map[d];}); }
function faMoney(n){ return toFa(Number(n||0).toLocaleString("en-US"))+" تومان"; }
async function api(path,opts){
  ...
  if(!res.ok){
    if(res.status===401 && me){ me=null; T.access=null; loginErr="نشست منقضی شد، دوباره وارد شو."; render(); }
    throw new Error((data&&data.error)||"خطای ارتباط با سرور");
  }
  return data;
}
var me=null, loginErr="", loginLoading=false;
var tab="dashboard";
var userSearch="", plansDraft=[], plansMsg="";
var aiDraft=null, aiStats=null, aiMsg="";
var dash=null, dashErr="";

var NAV=[
  {k:"dashboard",l:"داشبورد",i:"📊"},
  {k:"users",l:"کاربران",i:"👥"},
  {k:"vipcodes",l:"کدهای VIP",i:"🎁"},
  {k:"discounts",l:"کدهای تخفیف",i:"🏷️"},
  {k:"payments",l:"پرداخت‌ها",i:"💳"},
  {k:"plans",l:"پلن‌ها",i:"💎"},
  {k:"ai",l:"مربی AI",i:"🧠"},
  {k:"export",l:"خروجی",i:"📤"},
  {k:"roadmap",l:"نقشه راه",i:"🧭"}
];
...
function render(){
  ...
  var html='<div class="topbar"><div class="topbar-inner">'+
    '<div class="brand"><div class="badge">🛡️</div><div><div class="t1">پروتکل آیرون</div><div class="t2">پنل مدیریت</div></div></div>'+
    '<div class="row" style="gap:8px;flex-wrap:wrap;"><span class="role-pill">👑 '+esc(me.name||me.email||"مدیر")+'</span><button class="logout-btn" onclick="doLogout()">خروج</button></div>'+
  '</div></div>';
  html+='<div class="navtabs">'+NAV.map(function(n){ return '<button class="navbtn '+(tab===n.k?"active":"")+'" onclick="setTab(\''+n.k+'\')"><span>'+n.i+'</span><span>'+n.l+'</span></button>'; }).join("")+'</div>';
  html+='<div class="content" id="content">'+renderTab()+'</div>';
  app.innerHTML=html;
}
function setTab(k){ tab=k; render(); loadForTab(k); }
function renderTab(){
  if(tab==="dashboard") return renderDashboard();
  if(tab==="users") return renderUsers();
  if(tab==="vipcodes") return renderVipCodes();
...
function loadForTab(k){
  if(k==="dashboard") loadDashboard();
  else if(k==="users") loadUsers();
  else if(k==="vipcodes") loadVipCodes();
  else if(k==="discounts") loadDiscounts();
  else if(k==="payments") loadPayments();
...
function renderLogin(){
  return '<div class="login-wrap"><div class="panel login-card">'+
    '<div style="text-align:center;margin-bottom:18px;"><div class="badge" style="width:44px;height:44px;border:2px solid var(--brass);border-radius:12px;display:flex;align-items:center;justify-content:center;margin:0 auto 10px;font-size:20px;">🛡️</div>'+
    '<div style="font-weight:900;font-size:16px;">پنل مدیریت پروتکل آیرون</div></div>'+
    '<div class="field"><label>ایمیل مدیر</label><input id="loginEmail" type="text" placeholder="admin@example.com" /></div>'+
    '<div class="field"><label>رمز عبور</label><input id="loginPass" type="password" placeholder="••••••••" /></div>'+
    (loginErr?'<div class="err-banner">'+esc(loginErr)+'</div>':'')+
    '<button class="btn" style="width:100%;margin-top:14px;" onclick="doLogin()" '+(loginLoading?"disabled":"")+'>'+(loginLoading?"در حال ورود...":"ورود")+'</button>'+
  '</div></div>';
}
async function doLogin(){
  var email=document.getElementById("loginEmail").value.trim();
  var password=document.getElementById("loginPass").value;
  if(!email||!password){ loginErr="ایمیل و رمز عبور رو وارد کن"; render(); return; }
  loginLoading=true; loginErr=""; render();
  try{
    var res=await api("/api/auth/login",{method:"POST",body:JSON.stringify({email:email,password:password})});
    if(res.user.role!=="admin"){ loginErr="این حساب دسترسی مدیریت نداره."; loginLoading=false; render(); return; }
    T.access=res.accessToken; me=res.user; loginLoading=false;
    render(); loadForTab(tab);
  }catch(e){ loginErr=e.message; loginLoading=false; render(); }
}
function doLogout(){ me=null; T.access=null; tab="dashboard"; render(); }
...
async function loadDashboard(){
  ...
  try{
    ...
    var ai=await api("/api/admin/ai-stats");
    var payments=await api("/api/admin/payments");
    var successPayments=payments.filter(function(p){return p.status==="success";});
    var today=new Date().toISOString().slice(0,10);
    var thisMonth=today.slice(0,7);
    var dailyRevenueToman=successPayments.filter(function(p){return (p.paidAt||"").slice(0,10)===today;}).reduce(function(s,p){return s+(p.amountToman||0);},0);
    var monthlyRevenueToman=successPayments.filter(function(p){return (p.paidAt||"").slice(0,7)===thisMonth;}).reduce(function(s,p){return s+(p.amountToman||0);},0);
    dash={stats:stats,ai:ai,dailyRevenueToman:dailyRevenueToman,monthlyRevenueToman:monthlyRevenueToman,paymentsSampleSize:payments.length};
  }catch(e){ dashErr=e.message; }
  render();
}
function renderDashboard(){
  if(dashErr) return '<div class="err-banner">خطا در بارگذاری داشبورد: '+esc(dashErr)+' <button class="iconbtn2" style="margin-right:8px;" onclick="loadDashboard()">تلاش دوباره</button></div>';
  if(!dash) return '<div class="empty-note">در حال بارگذاری...</div>';
  var s=dash.stats, ai=dash.ai;
  var kpis=[
    {l:"کل کاربران",v:toFa(s.totalUsers),i:"👥"},
    {l:"کاربران VIP",v:toFa(s.vipUsers),i:"👑"},
    {l:"ثبت‌نام امروز",v:toFa(s.signupsToday),i:"🆕"},
    {l:"کد VIP ساخته‌شده",v:toFa(s.totalVipCodes),i:"🎁"},
    {l:"درآمد امروز",v:faMoney(dash.dailyRevenueToman),i:"💰"},
    {l:"درآمد این ماه",v:faMoney(dash.monthlyRevenueToman),i:"📈"},
    {l:"پیام‌های AI امروز",v:toFa(ai.todayMessages),i:"💬"},
    {l:"کلید AI",v:(ai.hasApiKey?"تنظیم شده":"تنظیم نشده"),i:"🧠"}
  ];
  var html='<div class="kpi-grid">'+kpis.map(function(k){ return '<div class="panel kpi-card"><div class="lb">'+k.i+' '+k.l+'</div><div class="val">'+k.v+'</div></div>'; }).join("")+'</div>';
  html+='<div style="font-size:10.5px;color:var(--muted);margin-top:10px;">درآمد بر اساس '+toFa(dash.paymentsSampleSize)+' تراکنش اخیر محاسبه شده (سقف نمایش سرور).</div>';
  return html;
}

/* ---------------- کاربران ---------------- */
var usersList=[], usersErr="";
async function loadUsers(){
  usersErr="";
  try{ usersList=await api("/api/admin/users?search="+encodeURIComponent(userSearch)); }
  catch(e){ usersErr=e.message; }
  var tb=document.getElementById("usersTbody");
  if(tb) tb.innerHTML=buildUsersRows(); else render();
}
function buildUsersRows(){
  if(usersErr) return '<tr><td colspan="6"><div class="err-banner">'+esc(usersErr)+'</div></td></tr>';
  if(!usersList.length) return '<tr><td colspan="6" class="empty-note">کاربری پیدا نشد</td></tr>';
  return usersList.map(function(u){
    var vipOn=u.vip&&u.vip.active;
    return '<tr><td>'+esc(u.name)+'</td><td class="mono" style="font-size:11px;">'+esc(u.email||u.phone||"—")+'</td>'+
      '<td>'+(u.role==="admin"?'<span class="pill warn">مدیر</span>':'<span class="pill muted">کاربر</span>')+'</td>'+
      '<td>'+(vipOn?'<span class="pill ok">فعال تا '+esc(new Date(u.vip.expiresAt).toLocaleDateString("fa-IR"))+'</span>':'<span class="pill muted">ندارد</span>')+'</td>'+
      '<td class="mono" style="font-size:11px;">'+esc((u.createdAt||"").slice(0,10))+'</td>'+
      '<td><div class="row" style="gap:4px;"><button class="iconbtn2" onclick="giftVip(\''+u.id+'\')">🎁 هدیه VIP</button>'+
      (vipOn?'<button class="iconbtn2" onclick="revokeVip(\''+u.id+'\')">✖️ لغو</button>':'')+'</div></td></tr>';
  }).join("");
}
function renderUsers(){
  return '<div class="panel section-gap" style="padding:14px;">'+
    '<div class="search-row"><input id="userSearchInput" placeholder="جستجو با نام، ایمیل یا موبایل..." value="'+esc(userSearch)+'" oninput="userSearch=this.value;loadUsers();" /></div>'+
    '<div class="table-wrap section-gap"><table class="dtable"><thead><tr><th>نام</th><th>ایمیل/موبایل</th><th>نقش</th><th>VIP</th><th>عضویت</th><th>عملیات</th></tr></thead>'+
    '<tbody id="usersTbody">'+buildUsersRows()+'</tbody></table></div>'+
  '</div>';
}
...
/* ---------------- کدهای VIP ---------------- */
var vipCodes=[], vipCodesErr="";
async function loadVipCodes(){
  vipCodesErr="";
  try{ vipCodes=await api("/api/admin/vip-codes"); }catch(e){ vipCodesErr=e.message; }
  var tb=document.getElementById("vipCodesTbody"); if(tb) tb.innerHTML=buildVipCodesRows(); else render();
}
function buildVipCodesRows(){
  if(vipCodesErr) return '<tr><td colspan="5"><div class="err-banner">'+esc(vipCodesErr)+'</div></td></tr>';
  if(!vipCodes.length) return '<tr><td colspan="5" class="empty-note">کدی ساخته نشده</td></tr>';
  return vipCodes.map(function(c){
    return '<tr><td class="mono">'+esc(c.code)+'</td><td>'+toFa(c.durationDays)+' روز</td>'+
      '<td>'+toFa(c.usedCount)+' / '+(c.maxUses===null?"∞":toFa(c.maxUses))+'</td>'+
      '<td>'+(c.isActive?'<span class="pill ok">فعال</span>':'<span class="pill muted">غیرفعال</span>')+'</td>'+
      '<td><button class="iconbtn2" onclick="toggleVipCode(\''+c.id+'\','+(!c.isActive)+')">'+(c.isActive?"غیرفعال کن":"فعال کن")+'</button></td></tr>';
  }).join("");
}
function renderVipCodes(){
  return '<div class="panel section-gap" style="padding:16px;">'+
    '<div style="font-weight:800;font-size:14px;margin-bottom:8px;">🎁 ساخت کد VIP جدید</div>'+
    '<div class="row" style="flex-wrap:wrap;gap:8px;">'+
      '<div class="field" style="flex:1;min-width:120px;"><label>کد دلخواه (اختیاری)</label><input id="vcCode" placeholder="خالی=خودکار" /></div>'+
      '<div class="field" style="flex:1;min-width:100px;"><label>مدت (روز)</label><input id="vcDays" type="number" placeholder="30" /></div>'+
      '<div class="field" style="flex:1;min-width:100px;"><label>سقف استفاده</label><input id="vcMax" type="number" placeholder="خالی=نامحدود" /></div>'+
    '</div>'+
    '<button class="btn" style="margin-top:10px;" onclick="createVipCode()">ساخت کد</button>'+
    '<div id="vcErr" class="err-banner" style="display:none;"></div>'+
  '</div>'+
  '<div class="panel section-gap" style="padding:14px;"><div class="table-wrap"><table class="dtable"><thead><tr><th>کد</th><th>مدت</th><th>استفاده</th><th>وضعیت</th><th>عملیات</th></tr></thead>'+
  '<tbody id="vipCodesTbody">'+buildVipCodesRows()+'</tbody></table></div></div>';
}
async function createVipCode(){
  var errEl=document.getElementById("vcErr"); errEl.style.display="none";
  try{
    var code=document.getElementById("vcCode").value;
    var durationDays=document.getElementById("vcDays").value;
    var maxUses=document.getElementById("vcMax").value;
    var created=await api("/api/admin/vip-codes",{method:"POST",body:JSON.stringify({code:code,durationDays:durationDays,maxUses:maxUses})});
    alert("کد ساخته شد: "+created.code);
    document.getElementById("vcCode").value=""; document.getElementById("vcDays").value=""; document.getElementById("vcMax").value="";
    loadVipCodes();
  }catch(e){ errEl.textContent=e.message; errEl.style.display="block"; }
}
async function toggleVipCode(id,val){
  try{ await api("/api/admin/vip-codes/"+id+"/toggle",{method:"PUT",body:JSON.stringify({isActive:val})}); loadVipCodes(); }
  catch(e){ alert(e.message); }
}

/* ---------------- کدهای تخفیف ---------------- */
var discCodes=[], discErr2="";
async function loadDiscounts(){
  discErr2="";
  try{ discCodes=await api("/api/admin/discount-codes"); }catch(e){ discErr2=e.message; }
  var tb=document.getElementById("discTbody"); if(tb) tb.innerHTML=buildDiscRows(); else render();
}
function buildDiscRows(){
  if(discErr2) return '<tr><td colspan="4"><div class="err-banner">'+esc(discErr2)+'</div></td></tr>';
  if(!discCodes.length) return '<tr><td colspan="4" class="empty-note">کد تخفیفی ساخته نشده</td></tr>';
  return discCodes.map(function(c){
    var disc=c.percentOff?(toFa(c.percentOff)+"٪"):(c.amountOff?faMoney(c.amountOff):"—");
    return '<tr><td class="mono">'+esc(c.code)+'</td><td>'+disc+'</td>'+
      '<td>'+toFa(c.usedCount)+' / '+(c.maxUses===null?"∞":toFa(c.maxUses))+'</td>'+
      '<td>'+(c.isActive?'<span class="pill ok">فعال</span>':'<span class="pill muted">غیرفعال</span>')+' <button class="iconbtn2" onclick="toggleDisc(\''+c.id+'\','+(!c.isActive)+')">'+(c.isActive?"غیرفعال کن":"فعال کن")+'</button></td></tr>';
  }).join("");
}
function renderDiscounts(){
  return '<div class="panel section-gap" style="padding:16px;">'+
    '<div style="font-weight:800;font-size:14px;margin-bottom:8px;">🏷️ ساخت کد تخفیف جدید</div>'+
    '<div class="row" style="flex-wrap:wrap;gap:8px;">'+
      '<div class="field" style="flex:1;min-width:120px;"><label>کد دلخواه</label><input id="dcCode" placeholder="خالی=خودکار" /></div>'+
      '<div class="field" style="flex:1;min-width:100px;"><label>درصد تخفیف</label><input id="dcPercent" type="number" placeholder="مثلاً 20" /></div>'+
      '<div class="field" style="flex:1;min-width:100px;"><label>یا مبلغ ثابت (تومان)</label><input id="dcAmount" type="number" /></div>'+
      '<div class="field" style="flex:1;min-width:100px;"><label>سقف استفاده</label><input id="dcMax" type="number" placeholder="خالی=نامحدود" /></div>'+
    '</div>'+
    '<button class="btn" style="margin-top:10px;" onclick="createDisc()">ساخت کد تخفیف</button>'+
    '<div id="dcErr" class="err-banner" style="display:none;"></div>'+
  '</div>'+
  '<div class="panel section-gap" style="padding:14px;"><div class="table-wrap"><table class="dtable"><thead><tr><th>کد</th><th>تخفیف</th><th>استفاده</th><th>وضعیت</th></tr></thead>'+
  '<tbody id="discTbody">'+buildDiscRows()+'</tbody></table></div></div>';
}
async function createDisc(){
  var errEl=document.getElementById("dcErr"); errEl.style.display="none";
  try{
    var code=document.getElementById("dcCode").value;
    var percentOff=document.getElementById("dcPercent").value;
    var amountOff=document.getElementById("dcAmount").value;
    var maxUses=document.getElementById("dcMax").value;
    var created=await api("/api/admin/discount-codes",{method:"POST",body:JSON.stringify({code:code,percentOff:percentOff,amountOff:amountOff,maxUses:maxUses})});
...
/* ---------------- پرداخت‌ها ---------------- */
var payList=[], payErr="", payFilter="pending";
async function loadPayments(){
  payErr="";
  try{ payList = payFilter==="pending" ? await api("/api/admin/payments/pending") : await api("/api/admin/payments"); }
  catch(e){ payErr=e.message; }
  render();
}
function renderPayments(){
  var html='<div class="row section-gap" style="gap:6px;">'+
    '<button class="iconbtn2 '+(payFilter==="pending"?"":"")+'" style="'+(payFilter==="pending"?"border-color:var(--brass);color:var(--brass2);":"")+'" onclick="payFilter=\'pending\';loadPayments();">در انتظار</button>'+
    '<button class="iconbtn2" style="'+(payFilter==="all"?"border-color:var(--brass);color:var(--brass2);":"")+'" onclick="payFilter=\'all\';loadPayments();">همه (۲۰۰ اخیر)</button>'+
  '</div>';
  if(payErr){ html+='<div class="err-banner section-gap">'+esc(payErr)+'</div>'; return html; }
  var statusMap={success:["ok","موفق"],pending:["warn","در انتظار"],failed:["bad","ناموفق"],rejected:["bad","رد شده"]};
  var rows=payList.map(function(p){
    var st=statusMap[p.status]||["muted",p.status];
    return '<tr><td class="mono" style="font-size:10px;">'+esc((p.userId||"").slice(0,8))+'</td>'+
      '<td>'+(p.provider==="usdt_wallet"?"تتر":p.provider==="zarinpal"?"زرین‌پال":esc(p.provider))+'</td>'+
      '<td>'+(p.amountUsdt?toFa(p.amountUsdt)+" USDT":(p.amountToman?faMoney(p.amountToman):"—"))+'</td>'+
      '<td style="font-size:10px;">'+esc(p.providerRef||"—")+'</td>'+
      '<td><span class="pill '+st[0]+'">'+st[1]+'</span></td>'+
      '<td>'+(p.status==="pending"?'<div class="row" style="gap:4px;"><button class="iconbtn2" onclick="approvePayment(\''+p.id+'\')">✔️ تایید</button><button class="iconbtn2" onclick="rejectPayment(\''+p.id+'\')">✖️ رد</button></div>':"—")+'</td></tr>';
  }).join("")||'<tr><td colspan="6" class="empty-note">تراکنشی نیست</td></tr>';
  html+='<div class="panel section-gap" style="padding:14px;"><div class="table-wrap"><table class="dtable"><thead><tr><th>کاربر</th><th>روش</th><th>مبلغ</th><th>کد تراکنش</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>'+rows+'</tbody></table></div></div>';
  return html;
}
async function approvePayment(id){
  if(!confirm("تایید بشه و VIP کاربر فعال بشه؟")) return;
  try{ await api("/api/admin/payments/"+id+"/approve",{method:"PUT"}); loadPayments(); }
  catch(e){ alert(e.message); }
}
async function rejectPayment(id){
  var reason=prompt("دلیل رد (اختیاری)","");
  try{ await api("/api/admin/payments/"+id+"/reject",{method:"PUT",body:JSON.stringify({reason:reason})}); loadPayments(); }
  catch(e){ alert(e.message); }
}
...
async function loadPlans(){
  plansErr=""; plansMsg="";
  try{ plansDraft=await api("/api/admin/plans"); }catch(e){ plansErr=e.message; }
  render();
}
function updatePlanDraft(id,field,val){
  var p=plansDraft.find(function(x){return x.id===id;});
  if(p) p[field]=val;
}
async function savePlan(id){
  ...
  if(!p) return;
  try{
    await api("/api/admin/plans/"+id,{method:"PUT",body:JSON.stringify({title:p.title,priceToman:Number(p.priceToman),priceUsdt:Number(p.priceUsdt),durationDays:Number(p.durationDays),isActive:p.isActive})});
    plansMsg="✅ پلن «"+p.title+"» ذخیره شد.";
  }catch(e){ plansMsg="⚠️ "+e.message; }
  render();
}
function togglePlanDraftActive(id){
  var p=plansDraft.find(function(x){return x.id===id;});
  if(p){ p.isActive=!p.isActive; render(); }
}
function renderPlans(){
  if(plansErr) return '<div class="err-banner section-gap">'+esc(plansErr)+' <button class="iconbtn2" onclick="loadPlans()">تلاش دوباره</button></div>';
  var html=(plansMsg?'<div class="panel section-gap" style="padding:10px 14px;font-size:12px;color:var(--muted);">'+esc(plansMsg)+'</div>':'');
  html+=plansDraft.map(function(p){
    return '<div class="panel section-gap" style="padding:14px;">'+
      '<div class="row" style="justify-content:space-between;">'+
        '<input value="'+esc(p.title)+'" oninput="updatePlanDraft(\''+p.id+'\',\'title\',this.value)" style="font-weight:800;font-size:13.5px;border:none;background:transparent;color:var(--text);flex:1;" />'+
        '<button class="iconbtn2" style="'+(p.isActive?"border-color:var(--green2);color:var(--green2);":"")+'" onclick="togglePlanDraftActive(\''+p.id+'\')">'+(p.isActive?"فعال":"غیرفعال")+'</button>'+
      '</div>'+
      '<div class="row" style="flex-wrap:wrap;margin-top:8px;">'+
        '<div class="field" style="flex:1;min-width:120px;"><label>قیمت تومان</label><input type="number" value="'+esc(p.priceToman)+'" oninput="updatePlanDraft(\''+p.id+'\',\'priceToman\',this.value)" /></div>'+
        '<div class="field" style="flex:1;min-width:100px;"><label>قیمت تتر (USDT)</label><input type="number" value="'+esc(p.priceUsdt)+'" oninput="updatePlanDraft(\''+p.id+'\',\'priceUsdt\',this.value)" /></div>'+
        '<div class="field" style="flex:1;min-width:100px;"><label>مدت (روز)</label><input type="number" value="'+esc(p.durationDays)+'" oninput="updatePlanDraft(\''+p.id+'\',\'durationDays\',this.value)" /></div>'+
      '</div>'+
      '<button class="btn" style="margin-top:10px;" onclick="savePlan(\''+p.id+'\')">💾 ذخیره این پلن</button>'+
    '</div>';
  }).join("");
  return html;
}
...
async function loadAi(){
  aiMsg="";
  try{ aiStats=await api("/api/admin/ai-stats"); aiDraft=Object.assign({},aiStats.settings); }
  catch(e){ aiMsg="خطا: "+e.message; }
  render();
}
async function saveAiSettings(){
  try{
    var enabled=document.getElementById("aiEnabled").checked;
    var freeDailyLimit=document.getElementById("aiFreeLimit").value;
    var model=document.getElementById("aiModel").value;
    await api("/api/admin/ai-settings",{method:"PUT",body:JSON.stringify({enabled:enabled,freeDailyLimit:freeDailyLimit,model:model})});
    aiMsg="✅ تنظیمات ذخیره شد.";
    loadAi();
  }catch(e){ aiMsg="⚠️ "+e.message; render(); }
}
function renderAi(){
  if(!aiStats) return '<div class="empty-note">در حال بارگذاری...</div>';
  var s=aiStats;
  var html='<div class="kpi-grid">'+
    [{l:"کلید API",v:(s.hasApiKey?"تنظیم شده":"تنظیم نشده"),i:"🔑"},
     {l:"پیام‌های امروز",v:toFa(s.todayMessages),i:"💬"},
     {l:"کل پیام‌ها",v:toFa(s.totalMessages),i:"📨"},
     {l:"کاربران فعال AI",v:toFa(s.distinctUsers),i:"👥"},
     {l:"کل توکن مصرفی",v:toFa(s.totalTokens),i:"🧮"}
    ].map(function(k){ return '<div class="panel kpi-card"><div class="lb">'+k.i+' '+k.l+'</div><div class="val" style="font-size:15px;">'+k.v+'</div></div>'; }).join("")+
  '</div>';
  html+='<div class="panel section-gap" style="padding:16px;">'+
    '<div style="font-weight:800;font-size:14px;margin-bottom:8px;">⚙️ تنظیمات مربی هوشمند</div>'+
    '<label style="font-size:12.5px;display:flex;align-items:center;gap:6px;"><input type="checkbox" id="aiEnabled" '+(aiDraft.enabled?"checked":"")+' /> مربی هوشمند فعال باشه</label>'+
    '<div class="field"><label>تعداد پیام رایگان روزانه</label><input id="aiFreeLimit" type="number" value="'+esc(aiDraft.freeDailyLimit)+'" /></div>'+
    '<div class="field"><label>مدل (مثلاً gemini-2.5-flash)</label><input id="aiModel" value="'+esc(aiDraft.model)+'" /></div>'+
    '<button class="btn" style="margin-top:10px;" onclick="saveAiSettings()">ذخیره تنظیمات</button>'+
    (aiMsg?'<div style="font-size:12px;margin-top:8px;color:var(--muted);">'+esc(aiMsg)+'</div>':'')+
  '</div>';
  return html;
}
...
function toCSV(rows,headers){
  function e2(v){ v=String(v==null?"":v); if(v.indexOf(",")>=0||v.indexOf('"')>=0||v.indexOf("\n")>=0) return '"'+v.replace(/"/g,'""')+'"'; return v; }
  var lines=[headers.map(function(h){return e2(h.l);}).join(",")];
  rows.forEach(function(r){ lines.push(headers.map(function(h){return e2(r[h.k]);}).join(",")); });
  return "\uFEFF"+lines.join("\n");
}
...
function exportUsers(){ downloadCSV("users.csv", toCSV(usersList,[{k:"name",l:"نام"},{k:"email",l:"ایمیل"},{k:"phone",l:"موبایل"},{k:"role",l:"نقش"},{k:"createdAt",l:"تاریخ عضویت"}])); }
function exportPayments(){ downloadCSV("payments.csv", toCSV(payList,[{k:"provider",l:"روش"},{k:"amountToman",l:"مبلغ تومان"},{k:"amountUsdt",l:"مبلغ تتر"},{k:"status",l:"وضعیت"},{k:"createdAt",l:"تاریخ"}])); }
function exportVipCodes(){ downloadCSV("vip_codes.csv", toCSV(vipCodes,[{k:"code",l:"کد"},{k:"durationDays",l:"مدت روز"},{k:"usedCount",l:"استفاده‌شده"},{k:"maxUses",l:"سقف"},{k:"isActive",l:"فعال"}])); }
function renderExport(){
  return '<div class="panel section-gap" style="padding:16px;">'+
    '<div style="font-weight:800;font-size:14px;margin-bottom:10px;">📤 خروجی گزارش‌ها (CSV)</div>'+
    '<div style="font-size:11.5px;color:var(--muted);margin-bottom:12px;">هر تب رو حداقل یک بار باز کن تا داده‌ش لود بشه، بعد از اینجا خروجی بگیر.</div>'+
    '<div class="row" style="flex-wrap:wrap;gap:8px;">'+
      '<button class="btn ghost" onclick="exportUsers()">⬇️ کاربران</button>'+
      '<button class="btn ghost" onclick="exportPayments()">⬇️ پرداخت‌ها</button>'+
      '<button class="btn ghost" onclick="exportVipCodes()">⬇️ کدهای VIP</button>'+
    '</div>'+
  '</div>';
}
...
function renderRoadmap(){
  var items=["سیستم تیکت پشتیبانی","ارسال اعلان همگانی/هدفمند به کاربران","احراز هویت دومرحله‌ای (2FA) برای ورود ادمین","IP Whitelist و لاگ کامل فعالیت‌های ادمین","مانیتورینگ زنده (کاربران آنلاین، وضعیت سرویس‌ها)","Heatmap و Funnel Analysis واقعی","A/B Testing برای قیمت و پیام‌های تبلیغاتی","دستیار هوش مصنوعی مخصوص مدیر (تحلیل و پیش‌بینی)"];
  return '<div class="panel section-gap" style="padding:16px;">'+
    '<div style="font-weight:800;font-size:14px;margin-bottom:4px;">🧭 نقشه راه</div>'+
    '<div style="font-size:11.5px;color:var(--muted);margin-bottom:10px;">این موارد هنوز روی بک‌اند پیاده نشدن.</div>'+
    items.map(function(t){ return '<div class="roadmap-item"><span>▫️</span><span>'+esc(t)+'</span></div>'; }).join("")+
  '</div>';
}
