...
function render(){
  ...
  if(canSee("payments")){ loadCodes(); loadPlans(); loadDiscounts(); loadPayments(); loadReferralSettings(); loadReferrals(); }
...
adminRoutes.get("/coach-wallets/:coachId", (req, res, next) => {
  try {
    ...
    const withdrawals = WithdrawalRepo.findByCoach(coach.id).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
...
adminRoutes.get("/coach-wallets/withdrawals/list", (req, res, next) => {
  try {
    ...
    list = list.map((w) => {
      ...
      return Object.assign({}, w, { coachName: coach ? coach.name : "-", coachUsername: u ? u.username : null });
    });
    res.json(list.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
  } catch (e) { next(e); }
});
...
coachRoutes.get("/wallet/withdrawals", (req, res, next) => {
  try {
    const coach = CoachRepo.findByUserId(req.userId);
    if (!coach) throw httpError(400, "شما مربی نیستید");
    res.json(WithdrawalRepo.findByCoach(coach.id).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
  } catch (e) { next(e); }
});
...
coachRoutes.post("/wallet/withdraw", (req, res, next) => {
  try {
    ...
    const wr = WithdrawalRepo.create({
      ...
      bankInfo: {
        cardNumber: method === "card" ? String(cardNumber).replace(/[\s-]/g, "") : null,
        sheba: method === "sheba" ? String(sheba).replace(/\s/g, "").toUpperCase() : null,
    ...
    res.status(201).json({ ok: true, withdrawal: wr, balance: w.balance });
  } catch (e) { next(e); }
});
